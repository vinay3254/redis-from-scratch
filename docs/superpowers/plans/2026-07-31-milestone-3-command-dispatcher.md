...
- [ ] **Step 1: Write `src/db.rs` with `Value` enum and `Db` struct**
- [ ] **Step 2: Add unit tests in `src/db.rs`**
...
- [ ] **Step 1: Write string commands (`PING`, `ECHO`, `SET`, `GET`)**
- [ ] **Step 2: Write generic commands (`DEL`, `EXISTS`)**
- [ ] **Step 3: Write dispatcher module and unit tests**
...
- [ ] **Step 1: Share `Arc<Mutex<Db>>` across connection threads and execute dispatched commands**
- [ ] **Step 2: Run unit tests with `cargo test`**
- [ ] **Step 3: Build server with `cargo check`**
