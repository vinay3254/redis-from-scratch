...
- [x] **Step 1: Write `src/persistence/rdb.rs` with `dump_db` and `load_db`**
- [x] **Step 2: Add unit tests for RDB binary roundtrips**

---

### Task 2: Implement `SAVE` and `BGSAVE` commands

- [x] **Step 1: Implement `save` and `bgsave` handlers in `src/commands/generic.rs`**
- [x] **Step 2: Register handlers in `src/commands/mod.rs` dispatcher**
- [x] **Step 3: Add unit tests for `SAVE` and `BGSAVE`**

---

### Task 3: Load `dump.rdb` on startup in `src/main.rs`

- [x] **Step 1: Update `src/main.rs` boot sequence to load `dump.rdb` if present**
...
- [x] **Step 1: Push `phase-9/rdb-persistence` to GitHub**
- [x] **Step 2: Open PR #9 against `phase-8/zsets`**
